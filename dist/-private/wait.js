 ...
 */
function beginWait(waiter, label) {
  ...
  const timeout = setTimeout(() => {
    console.warn(`ember-maplibre-gl: ${label} did not finish within ${WAIT_TIMEOUT_MS}ms, ` + `so anything waiting on it carries on without it.`);
    endWait();
  }, WAIT_TIMEOUT_MS);
  function endWait() {
    if (!waiting) return;
