...
class MapLibreGL extends Component {
  /** @internal */mapLibrary = this.args.mapLib || Map$1;
  /** @internal */
  static {
  ...
  /** @internal */registerElement = modifier((element, [options]) => {
    ...
    const onError = event => {
      ...
      this.error = event.error instanceof Error ? event.error : new Error(event.error?.message ?? 'MapLibre GL error');
    ...
    registerDestructor(this, () => {
      ...
      if (this.args.reuseMaps && this.map && styleUrl) {
        ...
        try {
          ...
          /* ignore */}
        this.map._resizeObserver?.disconnect();
